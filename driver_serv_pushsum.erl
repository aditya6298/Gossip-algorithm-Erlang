...
init([Num, Topo, Algo, Start]) ->
    gen_server:cast(self(), {start}),
    {ok, #state{
        num_nodes = Num, topology = Topo, algorithm = Algo, start_time = Start
    }}.

done(Pid) ->
    gen_server:cast(driver_serv_pushsum, {done, Pid}).
